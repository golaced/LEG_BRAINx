	...
	switch(state[0]){
		...
		case 1:
			...
			{cnt_over[0]-=2;state[0]=0;}
				...
				if((flag[0]>0&&Pit_fc<-ERO_ANGLE)||(flag[0]<0&&Pit_fc>ERO_ANGLE))
		...
		case 1:
			...
			{cnt_over[1]-=2;state[1]=0;}
				...
				if((flag[1]>0&&Rol_fc<-ERO_ANGLE)||(flag[1]<0&&Rol_fc>ERO_ANGLE))
