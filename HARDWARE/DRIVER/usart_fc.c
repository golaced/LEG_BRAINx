...
{ u8 id;
	...
	if(!(sum==*(data_buf+num-1)))		return;		//判断sum
	if(!(*(data_buf)==0xAA && *(data_buf+1)==0xAF))		return;		//判断帧头
	if(*(data_buf+2)<=4&&*(data_buf+2)>=1)
  { id=*(data_buf+2);
	  leg[id].leg_connect=1;
		leg[id].leg_loss_cnt=0;
	  leg[id].pos_now[0].x=(float)((int16_t)(*(data_buf+4)<<8)|*(data_buf+5))/10;
		leg[id].pos_now[0].y=(float)((int16_t)(*(data_buf+6)<<8)|*(data_buf+7))/10;
	  leg[id].pos_now[0].z=(float)((int16_t)(*(data_buf+8)<<8)|*(data_buf+9))/10;
		leg[id].pos_now[1].x=(float)((int16_t)(*(data_buf+10)<<8)|*(data_buf+11))/10;
		leg[id].pos_now[1].y=(float)((int16_t)(*(data_buf+12)<<8)|*(data_buf+13))/10;
		leg[id].pos_now[1].z=(float)((int16_t)(*(data_buf+14)<<8)|*(data_buf+15))/10;
		leg[id].pos_now[2].x=(float)((int16_t)(*(data_buf+16)<<8)|*(data_buf+17))/10;
		leg[id].pos_now[2].y=(float)((int16_t)(*(data_buf+18)<<8)|*(data_buf+19)/10);
		leg[id].pos_now[2].z=(float)((int16_t)(*(data_buf+20)<<8)|*(data_buf+21))/10;
		leg[id].sita[0]		 =(float)((int16_t)(*(data_buf+22)<<8)|*(data_buf+23))/10;
		leg[id].sita[1]		 =(float)((int16_t)(*(data_buf+24)<<8)|*(data_buf+25))/10;
		leg[id].sita[2]		 =(float)((int16_t)(*(data_buf+26)<<8)|*(data_buf+27))/10;
	  leg[id].leg_ground		 							   =*(data_buf+28);
	  leg[id].err		 					 						 =*(data_buf+29);
	}		
}
...
{u8 i;	u8 sum = 0;
  ...
  data_to_send[_cnt++]=0xAA;
	...
	data_to_send[_cnt++]=BYTE1(_temp);
	data_to_send[_cnt++]=BYTE0(_temp);
  _temp = leg[sel].pos_tar[0].y*10;
	data_to_send[_cnt++]=BYTE1(_temp);
	data_to_send[_cnt++]=BYTE0(_temp);
	_temp = leg[sel].pos_tar[0].z*10;
	data_to_send[_cnt++]=BYTE1(_temp);
	data_to_send[_cnt++]=BYTE0(_temp);
